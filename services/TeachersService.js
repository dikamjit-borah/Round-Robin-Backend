...
exports.fetch_week_view = async (
  ...
  db_connect.query(
    q,
    [teacher_id, week_start_date, week_end_date],
    (err, result) => {
      if (err) throw err;
      else {
        console.log(result);
...
exports.insert_into_schedules = async (user_inputs, res) => {
  ...
  db_connect.query(q1, [user_inputs[1], user_inputs[4]], (err, result) => {
    if (err) throw err;
    else {
      console.log("user_id", user_inputs[1]);
...
function checkIfValidTime(arrayOfTimes, scheduledStart, scheduledEnd) {
  ...
  console.log(`user schedule from - ${scheduledStart24} - ${scheduledEnd24}`);
  for (let i = 0; i < arrayOfTimes.length; i++) {
    ...
    console.log(`comparing for ${start24} - ${end24}`);
